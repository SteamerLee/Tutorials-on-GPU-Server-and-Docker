...
import os
import gc
from graph_gen import convergence_graph, paraTrend_graph
...
def run_process(process_no, selected_tasks):
    ...
    for kwargs in selected_tasks:
        algo_dicts = {'GA': GA, 'PSO': PSO, 'CSDE': CSDE}
        ...
        for time_no in range(run_times):
            ...
            if name == 'GA':
                ...
                algo = Algo(func=func, dim=problem_size, pop=pop, max_iter=iterations, lb= (np.zeros(problem_size) + bound[0]).tolist(), ub=(np.zeros(problem_size) + bound[1]).tolist())
                ...
                algo = Algo(func=func, dim=problem_size, bound=bound, max_iters=iterations, pop_size=pop,
                            func_name=func_name, show_info=verbose)
            ...
            if name == 'CSDE':
                x_pd = {'iteration': np.arange(len(fit_history)), 'fitness': fit_history, 'gbest_op': algo.gbest_op,
                        'CurrentToPbest_pop_size': algo.op_stat['CurrentToPbest'],
                        'PbestToRand_pop_size': algo.op_stat['PbestToRand']}

                x_pd = pd.DataFrame(x_pd, columns=['iteration', 'fitness', 'gbest_op', 'CurrentToPbest_pop_size',
                                                   'PbestToRand_pop_size'])

                x_pd.to_csv(path + str(time_no + 1) + '.csv', index=False)
            else:
            ...
            try:
                ...
                if name == 'CSDE':
                    SR_dict = algo.get_Rec()
                    FP = algo.FP
                    paraTrend_graph(dataLst=SR_dict, para='SR', func_name=func_name, problem_size=problem_size, iter_lst=iter_lst, root_dir=root_dir, algo_name=name,
                                    time_no=time_no, FP=FP)
